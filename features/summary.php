        ...
        foreach($data as $key => $category){
            ...
            if ((count($category->top) > 0) and (count($category->top) > 0)){
                ...
                foreach($category->top as $topkey => $top){
                    if ($topkey == 0){
                        $text .= "Onze school wordt door relatief veel ouders gewaardeerd ten aanzien van '".html_entity_decode($top[1])."' (".round($top[2]*100)."% van de ouders is hierover tevreden).";
                    }
                    if ($topkey == 1){
                        $text .= "Ook zijn relatief veel ouders tevreden over '".html_entity_decode($top[1])."' (".round($top[2]*100)."%).";
                    }
